...
export async function POST(req: NextRequest) {
  try {
    ...
    const uniqueName = `product-${randomBytes(12).toString("hex")}${ext}`;

    const uploadDir = join(process.cwd(), "storage", "uploads", "products");
    ...
    return Response.json({ error: "Upload foto produk gagal" }, { status: 500 });
  }
}
