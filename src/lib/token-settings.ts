    ...
    data: {
      conversions: {
        create: missingDefaults.map((conversion) => ({
          ...
          longDescription: (conversion as any).longDescription ?? null,
          icon: (conversion as any).icon ?? null,
    ...
    conversions:
      config.conversions.length > 0
        ? config.conversions.map((conversion) => ({
            ...
            longDescription: (conversion as any).longDescription,
            icon: (conversion as any).icon,
            isActive: conversion.isActive,
            sortOrder: conversion.sortOrder,
          }))
